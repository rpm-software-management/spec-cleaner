...
class RpmCopyright(RpmSection):
    ...
    def _add_pkg_header(self):
        specname = os.path.splitext(os.path.basename(spec))[0]
        self.lines.append('''#
...
class RpmSpecCleaner:
    ...
    def __init__(self, specfile, output, inline, force, diff, diff_prog, glob_macrofuncs):
...
The results it produces should always be checked by someone since it is not and will never be perfect.''')
    ...
    try:
        cleaner = RpmSpecCleaner(spec, options.output, options.inline, options.force, options.diff, options.diff_prog, global_macrofuncs)
        cleaner.run()
